...
$main_url=PATH."/library/twilio/handlers/handle_incoming_call1.php"; // URL of the same page.
...
if($level===-1 || $campaign_id===-1 || $action===11) // If the request came directly via call. OR if it is action 11 (IVR reply)
{
	if($action!==11)
	{
		...
		log_request("New call");
		log_request($campaign_details);
		$campaign_id = $campaign_details['camp_id'];
		//log_request("*** campaign id : ".$campaign_id);
	...
	else
	{
		$location_of_number = $campaignObject->getLocation($called_number);
		$location = $location_of_number['city_name'];
		//log_request("Checking case 11: digit: ".$digit);
		$arr_key_exist = $campaignObject->check_key($campaign_id,$level,$digit);
		//log_request($arr_key_exist);
		...
		// Check for the valid digit
		/*if($wrong_key)
		{
		}
		else
		{
			//CHECK ACTION TYPE ID OF THE KEY FROM THE DATABASE
			//CHANGE ACTION
			$action=10;
		}*/
...
/*	$qrystring="?campaign_id={$campaign_id}&level={$level}";
	$qrystring=parseAmp($qrystring);
	$message="Press 1 for this and 2 for that";
    echo   "<Response>
				<Gather action='{$main_url}{$qrystring}' numDigits='1'>
					<Say>{$message}</Say>
				</Gather>
				{$no_input_str}
			</Response>";*/
...
{
//	var_dump($wrong_key_message);exit;
	$level=$level+1;
...
else if($action===30) // If action is Verify the location of caller
{
	...
	$set_message = "<Say>{$message}</Say>";
	//$message="We see that you are calling from {$location}, if this is correct please press 1, if not press 2";
...
{
	//$message="Please wait while we connect your call to call-center.";
