...
var error = {
	...
	"Campaign": {
		...
		"valid_postback_url" : "Please enter valid postback url!",
...
function userLogout()
{
	...
	{
		var urlPattern = /^(https?|ftp):\/\/(((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:)*@)?(((\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5]))|((([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.)+(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.?)(:\d*)?)(\/((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)+(\/(([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)*)*)?)?(\?((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)|[\uE000-\uF8FF]|\/|\?)*)?(\#((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)|\/|\?)*)?$/
		...
		else if(!urlPattern.test($('#offer_url').val()))
		{
		...
		else if($.trim($('#call_center_number').val()) == "")
		{
        	alert(error.Campaign.call_center_number);
			$('#call_center_number').focus();				
		}
		/*else if(isNaN($('#call_center_number').val()))
		{
        	alert(error.Campaign.valid_call_center_number);
			$('#call_center_number').focus();				
		}*/
		else if($('#call_center_number').val().length < 10)
		{
			alert(error.Campaign.ten_digit_only);
			$('#call_center_number').focus();				
		}
		else if($('#deallocation_time').val() != "" && isNaN($('#deallocation_time').val())) //$('#deallocation_time').val() != "" &&
		{
		...
		else if($('#user_id').val() == 0)
		{
			...
			var datastr = "";
			var fields = new Array("campaign_id","name","offer_url","description","status","extra_number","user_id","call_center_number","deallocation_time");
			for(var i=0;i<fields.length;i++){
				datastr+=fields[i]+"=";
				datastr+=$("#"+fields[i]).val()+"&";
			}
			datastr+="page="+page+"&function="+func+"&step="+step;
