            ...
            <div class="widget_inside">
				<div class="col_12">
                	<div class="show_links">
                        <a href="javascript:show_records(0, 'users', 'view_manager')" <? if(isset($data['show_status']))if($data['show_status'] == 0) {?>style="color:black;"<? } ?>>All(<?=$data['rec_counts']['all']?>)</a><span> | </span>
                        <a href="javascript:show_records(1, 'users', 'view_manager')" <? if(isset($data['show_status']))if($data['show_status'] == 1) {?>style="color:black;"<? } ?>>Active(<?=$data['rec_counts']['active']?>)</a><span> | </span>
                        <a href="javascript:show_records(2, 'users', 'view_manager')" <? if(isset($data['show_status']))if($data['show_status'] == 2) {?>style="color:black;"<? } ?>>Deleted(<?=$data['rec_counts']['deleted']?>)</a>
                    </div>
                    <?php if($data['arr_permission'][0]['add_perm'] == 1) { ?>
                    <a href="javascript:showForm('0','users','show_manager_form');" class="for_links">Add Manager</a>
                    <?php } ?>
                                    ...
                                    if($value['is_active'] != 0){
										...
										if(isset($_POST['show_status'])){
											if($data['arr_permission'][0]['restore_perm'] == 1) { 
	                                        	echo "<a href=\"javascript:restoreEntry('".$value['user_id']."','users','restore_manager')\" class=\"tiptip-top\" title=\"Restore\"><img src=\"img/Restore_Value.png\" alt=\"restore\"></a>";
											}
										}
                                    }	
                                    echo "</td></tr>";
