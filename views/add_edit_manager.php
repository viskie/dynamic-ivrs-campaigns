...
<div class="container" id="actualbody">
	<div class="row">
    	<div class="widget clearfix">
    		<h2><?php if(isset($data['userDetails'])) echo " Edit Manager"; else echo "Add Manager"; ?></h2>
           <?php
                                         ...
                                         <table class='zebra'>
											...
											foreach($data['assignedUsers'] as $user){
                                                echo"<tr class='gradeX'>";
                                                    if($is_exist == true){
														if(count($data['checkedUsers']) > 0){
															foreach($data['checkedUsers'] as $key=>$value){
																if($user['user_id'] == $value)
																	echo"<td><input type='checkbox' name='user_array[]' value='".$user['user_id']."' checked='checked'/></td>";	
																else
																	echo"<td><input type='checkbox' name='user_array[]' value='".$user['user_id']."'/></td>";	
															}
                                                        }else{
															echo"<td><input type='checkbox' name='user_array[]' value='".$user['user_id']."'/></td>";	
														}
                                                    }else{
                                                        if($user['user_manager'] != 0)
                                                            echo"<td><input type='checkbox' name='user_array[]' value='".$user['user_id']."' checked='checked'/></td>";
                                                        else
                                                            echo"<td><input type='checkbox' name='user_array[]' value='".$user['user_id']."'/></td>";
                                                    }
                                                     echo"<td class=''>".$user['user_name']."</td>";	
            ...
            <div class="clearfix grey-highlight">
                ...
                <?php if(isset($data['userDetails'])){
					...
					echo "<input type=\"submit\" class=\"button grey\" value=\"Cancel\" onclick=\"callPage('users','view_manager')\"></input>";	
                ?>
                </div>
            </div>
		</div>
